...
function handleUserQuery(userQuery, userQueryHTML, imgUrlPath) {
    logDebug('Processing user query:', userQuery);
    logDebug('Session active:', sessionActive);
    logDebug('WebRTC connection state:', peerConnection ? peerConnection.iceConnectionState : 'no connection');
...
window.microphone = async () => {
    if (document.getElementById('microphone').innerHTML === 'Stop Microphone') {
        ...
        speechRecognizer.stopContinuousRecognitionAsync(
            () => {
                logDebug('Microphone stopped successfully');
...
speechRecognizer.properties.setProperty("SpeechServiceResponse_RequestWordLevelTimestamps", "true");
    ...
    // Add speech recognizer event handlers with logging
    speechRecognizer.recognizing = (s, e) => {
        logDebug('Speech recognizing:', e.result.text);
    };

    speechRecognizer.recognized = async (s, e) => {
        if (e.result.reason === SpeechSDK.ResultReason.RecognizedSpeech) {
            let userQuery = e.result.text.trim();
            logDebug('Speech recognized:', userQuery);
            
            if (userQuery === '') {
                logDebug('Empty query detected, ignoring');
                return;
            }

            if (!document.getElementById('continuousConversation').checked) {
                logDebug('Single conversation mode - stopping microphone');
                document.getElementById('microphone').disabled = true;
                speechRecognizer.stopContinuousRecognitionAsync(
                    () => {
                        logDebug('Microphone stopped after recognition');
                        document.getElementById('microphone').innerHTML = 'Start Microphone';
                        document.getElementById('microphone').disabled = false;
                    },
                    (err) => {
            ...
            logDebug('Recognition result:', e.result.reason);
        }
    };

    speechRecognizer.canceled = (s, e) => {
        logError('Speech recognition canceled:', e.errorDetails);
    };

    // Start recognition with logging
    ...
    if (document.getElementById('useLocalVideoForIdle').checked) {
        if (!sessionActive) {
            logDebug('Session not active, connecting avatar...');
            connectAvatar();
        }
        setTimeout(() => {
            logDebug('Playing audio...');
            document.getElementById('audioPlayer').play()
                .catch(err => logError('Audio playback failed:', err));
        }, 5000);
    } else {
        logDebug('Playing audio directly');
        document.getElementById('audioPlayer').play()
            .catch(err => logError('Audio playback failed:', err));
    }
}
